...
double initTime;
double timeLastedFromInit;
double time_sweep_start;
double time_last_sweep_start;
...
boost::shared_ptr<ros::Publisher> freq_pub1, freq_pub2;
#define FREQ_CHECK(pub_name) pub_name->publish(std_msgs::Int32())
...
pcl::PointCloud<pcl::PointXYZHSV>::Ptr laserCloudCornerLLast(new pcl::PointCloud<pcl::PointXYZHSV>());//LastLast
pcl::PointCloud<pcl::PointXYZHSV>::Ptr laserCloudSurfLLast(new pcl::PointCloud<pcl::PointXYZHSV>());//LastLast
...
void TransformReset()
{
  ...
  transformRec[3] -= imuVeloFromStartXLast * (time_sweep_start - time_last_sweep_start);
  transformRec[4] -= imuVeloFromStartYLast * (time_sweep_start - time_last_sweep_start);
  transformRec[5] -= imuVeloFromStartZLast * (time_sweep_start - time_last_sweep_start);
}
...
/*
Just extracting feature point cloud(already extracted from raw point cloud) and imu data from topic.
*/
void laserCloudExtreCurHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudExtreCur2)
{
  ...
  timeLastedFromInit = timeLaserCloudExtreCur - initTime;
  ...
  //seperate imu data from feature points cloud...
  //looks very unefficient.
  ...
  if (!imuInited) {
    ...
    //transformSum[1] += imuYawStartCur;  //TODO : why don't accumulate yaw?
    transformSum[2] += imuRollStartCur;

    imuInited = true;
  }

  if (timeLastedFromInit > 4.0) {
    //look like the signal for starting laser odometry algorithm.
    //maybe waiting for first point cloud set to be received.
    newLaserCloudExtreCur = true;
  }
  else
  {
    //Only once...
    ROS_ERROR("Waiting for 'timeLastedFromInit' become larger than 4.0");
  }
}

/*
 * Extract last corner and plane points and build kd-tree for fast searching.
 *
 */
void laserCloudLastHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudLast2)
{
  ...
  if (laserCloudLast2->header.stamp.toSec() > timeLaserCloudLast + 0.005) {
    //1 Hz
    //<editor-fold desc="new sweep has started">
    timeLaserCloudLast = laserCloudLast2->header.stamp.toSec();
    time_last_sweep_start = time_sweep_start;
    time_sweep_start = timeLaserCloudLast - initTime;

    //Caching last features
...
int main(int argc, char** argv)
{
  ...
  freq_pub1.reset(new ros::Publisher(nh.advertise<std_msgs::Int32> ("/FreqCheck1", 20)));
  freq_pub2.reset(new ros::Publisher(nh.advertise<std_msgs::Int32> ("/FreqCheck2", 20)));
  ...
  while (status) {
    ...
    pcl::PointCloud<pcl::PointXYZHSV>::Ptr featurePoints, cornerDatabase, surfaceDatabase;
    pcl::KdTreeFLANN<pcl::PointXYZHSV>::Ptr kdtree_corner, kdtree_surface;
    if (newLaserCloudExtreCur && newLaserCloudLast)
    {
      // 1 Hz
      //<editor-fold desc="New sweep has just been started">
      startTime = time_last_sweep_start;
      endTime = time_sweep_start;

      // TODO : why use point cloud 2 steps before?
      featurePoints = laserCloudExtreLast;
      cornerDatabase = laserCloudCornerLLast;
      surfaceDatabase = laserCloudSurfLLast;
      kdtree_corner = kdtreeCornerLLast;
      kdtree_surface = kdtreeSurfLLast;
      ...
      if (laserCloudSurfLLast->points.size() >= 100) {
        sufficientPoints = true;  //set flag
      }
      //</editor-fold>
    }
    else if (newLaserCloudExtreCur)
    {
      ...
      static double timeLastedPreviousStep = 0.0;

      startTime = time_sweep_start;
      endTime = timeLastedFromInit;

      featurePoints = laserCloudExtreCur;
      cornerDatabase = laserCloudCornerLast;
      surfaceDatabase = laserCloudSurfLast;
      kdtree_corner = kdtreeCornerLast;
      kdtree_surface = kdtreeSurfLast;
      ...
      float s = (timeLastedFromInit - timeLastedPreviousStep) / (time_sweep_start - time_last_sweep_start);
      for (int i = 0; i < 6; i++) {
        transform[i] += s * transformRec[i]; //TODO : what is this? I geuss it is equation (4).
      }
      timeLastedPreviousStep = timeLastedFromInit;
      newLaserPoints = true;
    ...
    if (newLaserPoints && sufficientPoints)
    {
      ...
      int featuresNum = featurePoints->points.size();
      int laserCloudCornerNum = cornerDatabase->points.size();
      int laserCloudSurfNum = surfaceDatabase->points.size();

      float st = 1;
      if (!sweepEnd) {
        st = (timeLastedFromInit - startTime) / (time_sweep_start - time_last_sweep_start);
      }
      int iterNum = st * 50;
      ...
      for (int iterCount = 0; iterCount < iterNum; iterCount++) {
        ...
        for (int i = 0; i < featuresNum; i++) {
          //<editor-fold desc="Description">
          extreOri = featurePoints->points[i];
          ...
          if (fabs(extreOri.v + 1) < 0.05) {
            //<editor-fold desc="Find Surface Correspondence">
            int closestPointInd = -1, minPointInd2 = -1, minPointInd3 = -1;
            if (isPointSel) {
              //<editor-fold desc="Description">
              kdtree_surface->nearestKSearch(extreSel, 1, pointSearchInd, pointSearchSqDis);
              if (pointSearchSqDis[0] > 1.0) {
                continue;
              }

              closestPointInd = pointSearchInd[0];
              float closestPointTime = surfaceDatabase->points[closestPointInd].h;
              float pointSqDis, minPointSqDis2 = 1, minPointSqDis3 = 1;
              for (int j = closestPointInd + 1; j < laserCloudSurfNum; j++) {
                if (surfaceDatabase->points[j].h > closestPointTime + 0.07) {
                  break;
                }

                pointSqDis = (surfaceDatabase->points[j].x - extreSel.x) *
                             (surfaceDatabase->points[j].x - extreSel.x) +
                             (surfaceDatabase->points[j].y - extreSel.y) *
                             (surfaceDatabase->points[j].y - extreSel.y) +
                             (surfaceDatabase->points[j].z - extreSel.z) *
                             (surfaceDatabase->points[j].z - extreSel.z);

                if (surfaceDatabase->points[j].h < closestPointTime + 0.005) {
              ...
              if (pointSelInd[3 * i] >= 0) {
                ...
                float distX = extreSel.x - surfaceDatabase->points[closestPointInd].x;
                float distY = extreSel.y - surfaceDatabase->points[closestPointInd].y;
                float distZ = extreSel.z - surfaceDatabase->points[closestPointInd].z;
            ...
            if (minPointInd2 >= 0 && minPointInd3 >= 0) {
              //<editor-fold desc="Description">
              tripod1 = surfaceDatabase->points[closestPointInd];
              tripod2 = surfaceDatabase->points[minPointInd2];
              tripod3 = surfaceDatabase->points[minPointInd3];
          ...
          {
            //<editor-fold desc="Find Edge Correspondence">
            int closestPointInd = -1, minPointInd2 = -1;
            if (isPointSel) {
              //<editor-fold desc="Description">
              kdtree_corner->nearestKSearch(extreSel, 1, pointSearchInd, pointSearchSqDis);
              if (pointSearchSqDis[0] > 1.0) {
                continue;
              }

              closestPointInd = pointSearchInd[0];
              float closestPointTime = cornerDatabase->points[closestPointInd].h;

              float pointSqDis, minPointSqDis2 = 1;
              for (int j = closestPointInd + 1; j < laserCloudCornerNum; j++) {
                if (cornerDatabase->points[j].h > closestPointTime + 0.07) {
                  break;
                }

                pointSqDis = (cornerDatabase->points[j].x - extreSel.x) *
                             (cornerDatabase->points[j].x - extreSel.x) +
                             (cornerDatabase->points[j].y - extreSel.y) *
                             (cornerDatabase->points[j].y - extreSel.y) +
                             (cornerDatabase->points[j].z - extreSel.z) *
                             (cornerDatabase->points[j].z - extreSel.z);

                if (cornerDatabase->points[j].h > closestPointTime + 0.005) {
                  if (pointSqDis < minPointSqDis2) {
                    minPointSqDis2 = pointSqDis;
                    minPointInd2 = j;
                  }
              ...
              if (pointSelInd[3 * i] >= 0) {
                ...
                float distX = extreSel.x - cornerDatabase->points[closestPointInd].x;
                float distY = extreSel.y - cornerDatabase->points[closestPointInd].y;
                float distZ = extreSel.z - cornerDatabase->points[closestPointInd].z;
                if (distX * distX + distY * distY + distZ * distZ > 1.0) {
                  continue;
                }
              } else {
            ...
            if (minPointInd2 >= 0) {
              //<editor-fold desc="Description">
              tripod1 = cornerDatabase->points[closestPointInd];
              tripod2 = cornerDatabase->points[minPointInd2];
        ...
        matAtB = matAt * matB;
        cv::solve(matAtA, matAtB, matX, cv::DECOMP_QR); //cv::solve(matA, matB, matX, cv::DECOMP_SVD);
